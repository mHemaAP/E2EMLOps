...
class Sportprediction:
    def load_model(self)->None:
        self.session = ort.InferenceSession( os.path.join('checkpoints','onnxs','vegfruits.onnx') )
        self.session_input_name = self.session.get_inputs()[0].name
        self.classes = get_classes('vegfruits')
...
@app.post("/vegfruits",response_model=dict)
